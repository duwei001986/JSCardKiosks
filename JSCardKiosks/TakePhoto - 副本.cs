...
namespace JSCardKiosks
{
    public partial class TakePhoto : Form
    {
            ...
            switch (iStep)
            {
                ...
                case STEP_2_TAKEPHOTO:
                    ...
                    if (bHasReady)
                    {
                        ...
                        //imgPhoto = img;
                        if (iTime == 5)
                            oldImg1 = img; photo1.Image = imgPhoto;
                        if (iTime == 4)
                            oldImg2 = img; photo2.Image = imgPhoto;
                        labRetTime2.Text = (iTime - 2).ToString().PadLeft(2, '0');
                        PublicFuction.SoundPlay((iTime - 2).ToString() + ".wav");
                        if (iTime == 3)
                        {
                            oldImg3 = img; photo3.Image = imgPhoto;
        ...
        private void labBeauty_Click(object sender, EventArgs e)
        {
            ...
            {
                img = photo1.Image.Clone() as System.Drawing.Image;
            ...
            {
                img = photo2.Image.Clone() as System.Drawing.Image;
                BitmapData BmpDataS = ((Bitmap)img).LockBits(new Rectangle(0, 0, img.Width, img.Height), ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);
                PublicFuction.IM_NewMakeUp(BmpDataS.Scan0, BmpDataS.Scan0, img.Width, img.Height, BmpDataS.Stride, 2, 1, 2);
                ((Bitmap)img).UnlockBits(BmpDataS);
                photo2.Image = img;
            ...
            {
                img = photo3.Image.Clone() as System.Drawing.Image;
                BitmapData BmpDataS = ((Bitmap)img).LockBits(new Rectangle(0, 0, img.Width, img.Height), ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);
                PublicFuction.IM_NewMakeUp(BmpDataS.Scan0, BmpDataS.Scan0, img.Width, img.Height, BmpDataS.Stride, 2, 1, 2);
                ((Bitmap)img).UnlockBits(BmpDataS);
                photo3.Image = img;
        ...
        private void labCancelBeauty_Click(object sender, EventArgs e)
        {
            if (btnChoose1.Visible)
            {
                photo1.Image = oldImg1;
            }
            if (btnChoose2.Visible)
            {
                photo2.Image = oldImg2;
            }
            if (btnChoose3.Visible)
            {
                photo3.Image = oldImg3;
            }
        }
        ...
        private void labsubmitPhoto_Click(object sender, EventArgs e)
        {
            ...
            if (iChoosePhoto ==1)
            {
                PublicFuction.cutScrPhoto(oldImg1, -oldImg1.Width / 2+ oldImg1.Height / 2, 0, oldImg1.Height, oldImg1.Height);
            ...
            if (iChoosePhoto == 2)
            {
                PublicFuction.cutScrPhoto(oldImg2, -oldImg2.Width / 2 + oldImg2.Height / 2, 0, oldImg2.Height  , oldImg2.Height);
                // PublicFuction.SavePhoto(oldImg2, path);
            }
            if (iChoosePhoto == 3)
            {
                PublicFuction.cutScrPhoto(oldImg3, -oldImg3.Width/2+oldImg3.Height/2, 0, oldImg3.Height , oldImg3.Height);
